...
class GatherSourcesTest(TaskTestBase):
  ...
  def _gather_sources(self, groups):
    ...
    partition = TargetsPartition.create_from_groups(groups)
    ...
    context.products.get_data(
        ...
        lambda: {group_id: interpreter for group_id in partition.group_ids()})
    ...
    return {partition.groups_by_id[group_id]: pex.cmdline()[1]
            for (group_id, pex) in sources.items()}
