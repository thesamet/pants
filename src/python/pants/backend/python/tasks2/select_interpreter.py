...
class SelectInterpreter(Task):
  ...
  Produces under PYTHON_INTERPRETERS a mapping between target roots and interpreters
  that matches the transitive constraints of these targets."""
  ...
  def execute(self):
    ...
    if self.context.products.is_required_data(self.PYTHON_INTERPRETERS):
      ...
      for group_id in partition.group_ids():
          interpreters[group_id] = _get_interpreter_for_target_set(partition.closure_for_group_id(
              group_id))
