...
class GatherSources(Task):
  """Gather local Python sources for each target roots.

  For each root target, this task creates an (unzipped) PEX on disk containing the
  local Python sources.  This PEX can be merged with a requirements PEX to create
  a unified Python environment for running the relevant python code.

  Produces PYTHON_SOURCES, a mapping between target roots and the corresponding pex.
  """
  ...
  def execute(self):
    ...
    for group_id in partition.group_ids():
      target_set = filter(has_python_sources, partition.closure_for_group_id(group_id))
      with self.invalidated(target_set) as invalidation_check:
         python_sources[group_id] = self._get_pex_for_versioned_targets(
             interpreters[group_id], invalidation_check.all_vts)
